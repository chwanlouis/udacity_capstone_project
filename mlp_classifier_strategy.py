...
class MLPClassifierStrategy(strategy.BacktestingStrategy):
    def __init__(self, feed, instrument, capital, selected_features):
        super(MLPClassifierStrategy, self).__init__(feed, capital)
    ...
    def bars_to_data_dict(self, bars):
        data_dict = dict()
        for instrument in self.instrument:
    ...
    def onHistoricalData(self, bars):
        data_dict = self.bars_to_data_dict(bars)
        self.historical_data.append(data_dict)

    def data_preprocess(self, historical_df, days=20):
        y_colnames = list()
    ...
    def training_model(self, X, y):
        ...
        clf = MLPClassifier(hidden_layer_sizes=(5,10), random_state=1)
        pipeline = Pipeline(
...
def main():
    ...
    tree_reg_strategy = MLPClassifierStrategy(
        feed=feed,
        instrument=instruments.keys(),
        capital=1000000.0,
        selected_features=['%s_close' % asset for asset in sorted(instruments.keys())]
    )
